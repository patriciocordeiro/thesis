...
Nesta seção são apresentados os resultados preliminares de simulações realizadas no ambiente Matlab para geração do SBD-BB. Esta etapa tem como objetivos: 1)
%\begin{itemize}
%\item 
Analisar a influência da escolha de diferentes conjuntos de $\{\theta\}$ nos valores de PAPR e RSR; 
%\item 
2) Validar a expressão da RSR proposta na Subseção~\ref{subsec:rsr}, através do levantamento das RSRs para diferentes conjuntos de Multitom.
...
Onde observa-se que o sinal no domínio do tempo (\textbf{parte superior da Figura}) apresenta altos picos em relação à média, principalmente nas duas extremidades. Como resultado, obteve-se um valor alto do PAPR, aproximadamente 10~dB. No domínio da frequência, o resultado é uma PSD altamente distorcida (\textbf{parte inferior da Figura}), dentro e fora da banda de interesse.
...
\caption{Comparação das características estimadas do SBD-BB com as teóricas (para $|{\cal K}|=0$ até $|{\cal K}|=\LTEsubportadorasrounded$), computados com todos os valores do conjunto $\{\theta\} =0$: \textbf{superior})  Potência do sinal. \textbf{médio})  Potência do erro de quantização. \textbf{inferior}) RSR.}%
...
\caption{Comparação das características estimadas do SBD-BB com as teóricas (para $|{\cal K}|=0$ até $|{\cal K}|=\LTEsubportadorasrounded$), computados com os valores do conjunto $\{{\theta}\}$ uniformemente distribuídos de $[-\pi,\pi]$: \textbf{superior})  Potência do sinal. \textbf{médio})  Potência do erro de quantização. \textbf{inferior}) RSR.}%
